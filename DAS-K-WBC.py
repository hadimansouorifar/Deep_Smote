...
def discriminator():
    model = Sequential()
    model.add(Dense(4, input_dim=9, init='uniform'))
    model.add(Dense(2))
...
def regression():
    model = Sequential()
    model.add(Dense(64, input_dim=18))
    model.add(Dense(48))
    model.add(Dense(32))
    model.add(Dense(24))
    model.add(Dense(12))
...
for mm in range(0,3):
    ...
    for train, test in kf.split(x):
        ...
        new = numpy.zeros((c2 - c1, 18), dtype=numpy.float)

        r10 = numpy.zeros((ntrain, 9), dtype=numpy.float)
        r20 = numpy.zeros((ntrain, 9), dtype=numpy.float)
        new2 = numpy.zeros((ntrain, 18), dtype=numpy.float)
