...
def discriminator():
    model = Sequential()
    model.add(Dense(16, input_dim=34, init='uniform'))
    model.add(Dense(8))
...
def regression():
    model = Sequential()
    model.add(Dense(136, input_dim=68))
    model.add(Dense(112))
    model.add(Dense(86))
    model.add(Dense(64))
    model.add(Dense(52))
    model.add(Dense(output_dim=34))
...
x = data[:, 0:34]  # select columns 1 through end
...
w = data[:, 34]   # select column 0, the stock price
...
for mm in range(0,3):
    ...
    for train, test in kf.split(x):
        ...
        ntrain = 8000
        majority = numpy.zeros((c2, 34), dtype=numpy.float)
        minority = numpy.zeros((c1, 34), dtype=numpy.float)
        r1 = numpy.zeros((c2 - c1, 34), dtype=numpy.float)
        r2 = numpy.zeros((c2 - c1, 34), dtype=numpy.float)
        cent = numpy.zeros((ntrain, 34), dtype=numpy.float)

        new = numpy.zeros((c2 - c1, 68), dtype=numpy.float)

        r10 = numpy.zeros((ntrain, 34), dtype=numpy.float)
        r20 = numpy.zeros((ntrain, 34), dtype=numpy.float)
        new2 = numpy.zeros((ntrain, 68), dtype=numpy.float)
