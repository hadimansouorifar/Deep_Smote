...
def discriminator():
    model= Sequential()
    model.add(Dense(20, input_dim=36, init='uniform'))
...
def regression():
    model = Sequential()
    model.add(Dense(96, input_dim=36))
    model.add(Dense(48))
    model.add(Dense(output_dim = 36))
...
for mm in range(0,3):
    ...
    for train, test in kf.split(x):
        ...
        r1 = numpy.zeros((c2 - c1, 9), dtype=numpy.float)
        r2 = numpy.zeros((c2 - c1, 9), dtype=numpy.float)
        cent = numpy.zeros((ntrain, 9), dtype=numpy.float)


        r10 = numpy.zeros((ntrain, 9), dtype=numpy.float)
        r20 = numpy.zeros((ntrain, 9), dtype=numpy.float)
        new2 = numpy.zeros((ntrain, 36), dtype=numpy.float)
        ...
        def train(X_train, epochs=100, batch=64, save_interval=200):
            ...
            for cnt in range(epochs):
                ...
                gen_noise = np.random.uniform(n1, n2, size=[len(X_train), 36])
                ...
                # train generator
                noise = np.random.normal(n1, n2, (10, 36))
        ...
        noise = np.random.normal(n1, n2, (len(majority) - len(minority), 36))
