...
def discriminator():
    model= Sequential()
    model.add(Dense(20, input_dim=32, init='uniform'))
...
def regression():
    model = Sequential()
    model.add(Dense(96, input_dim=32))
    model.add(Dense(48))
...
for mm in range(0,3):
    ...
    for train, test in kf.split(x):
        ...
        new2 = numpy.zeros((ntrain, 32), dtype=numpy.float)
        ...
        def train(X_train, epochs=100, batch=64, save_interval=200):
            ...
            for cnt in range(epochs):
                ...
                gen_noise = np.random.uniform(n1, n2, size=[len(X_train), 32])
                ...
                # train generator
                noise = np.random.normal(n1, n2, (10, 32))
        ...
        noise = np.random.normal(n1, n2, (len(majority) - len(minority), 32))
