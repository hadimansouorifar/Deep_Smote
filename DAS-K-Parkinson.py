...
def discriminator():
    model = Sequential()
    model.add(Dense(16, input_dim=22, init='uniform'))
    model.add(Dense(8))
...
def regression():
    model = Sequential()
    model.add(Dense(112, input_dim=44))
    model.add(Dense(96))
    model.add(Dense(82))
    model.add(Dense(64))
    model.add(Dense(32))
...
for mm in range(0,3):
    ...
    for train, test in kf.split(x):
        ...
        new = numpy.zeros((c2 - c1, 44), dtype=numpy.float)

        r10 = numpy.zeros((ntrain, 22), dtype=numpy.float)
        r20 = numpy.zeros((ntrain, 22), dtype=numpy.float)
        new2 = numpy.zeros((ntrain, 44), dtype=numpy.float)
