...
def discriminator():
    model= Sequential()
    model.add(Dense(44, input_dim=88, init='uniform'))
...
def regression():
    model = Sequential()
    model.add(Dense(368, input_dim=88))
    model.add(Dense(198))
...
for mm in range(0,3):
    ...
    for train, test in kf.split(x):
        ...
        majority = numpy.zeros((c2, 22), dtype=numpy.float)
        minority = numpy.zeros((c1, 22), dtype=numpy.float)
        r1 = numpy.zeros((c2 - c1, 22), dtype=numpy.float)
        r2 = numpy.zeros((c2 - c1, 22), dtype=numpy.float)
        cent = numpy.zeros((ntrain, 22), dtype=numpy.float)


        r10 = numpy.zeros((ntrain, 22), dtype=numpy.float)
        r20 = numpy.zeros((ntrain, 22), dtype=numpy.float)
        new2 = numpy.zeros((ntrain, 88), dtype=numpy.float)
        ...
        def train(X_train, epochs=100, batch=64, save_interval=200):
            gloss = []
            dloss = []
            epoch = []
            for cnt in range(epochs):
                ...
                gen_noise = np.random.uniform(n1, n2, size=[len(X_train), 88])
                ...
                # train generator
                noise = np.random.normal(n1, n2, (10, 88))
        ...
        noise = np.random.normal(n1, n2, (len(majority) - len(minority), 88))
